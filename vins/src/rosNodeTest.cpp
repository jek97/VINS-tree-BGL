...
int main(int argc, char **argv)
{
    ...
    n->declare_parameter("vins_config_file", " n");
    string bob = n->get_parameter("vins_config_file").as_string();
    printf("config_file 0: %s\n", bob.c_str());

    std::vector<rclcpp::Parameter> new_config{rclcpp::Parameter("vins_config_file", "world")};
    n->set_parameters(new_config);
    bob = n->get_parameter("vins_config_file").as_string();
    printf("config_file 1: %s\n", bob.c_str());
