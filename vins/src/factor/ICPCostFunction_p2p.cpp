#include "ICPCostFunction_p2p.h"
// ==============================================================================
// ICPCostFunction_p2p Implementation - Point-to-Point Version
// Reprojects anchor point to current frame for error computation
// WITH DEPTH OPTIMIZATION SUPPORT
// ==============================================================================

ICPCostFunction_p2p::ICPCostFunction_p2p(
...
template <typename T>
Eigen::Matrix<T, 4, 4> ICPCostFunction_p2p::poseToMatrix(const T* pose) const {
...
template <typename T>
bool ICPCostFunction_p2p::operator()(
    ...
    // Compute direct point-to-point distance vector
    // For point-to-point ICP, we simply compute the difference between the two points
    ...
    // This represents the full 3D Euclidean distance between matched points
    residuals[0] = weight * point_diff(0);  // x-component in camera frame j
    residuals[1] = weight * point_diff(1);  // y-component in camera frame j
    residuals[2] = weight * point_diff(2);  // z-component in camera frame j
    
    return true;
}

ceres::CostFunction* ICPCostFunction_p2p::Create(
    const Eigen::Vector3d& _pts_i,
    const Eigen::Vector3d& _velocity_i,
    double _td_i,
    const Eigen::Vector3d& _pts_j,
    ...
    // Point-to-point version: 3 residuals, parameter blocks: pose_i(7), pose_j(7), ex_pose(7), depth(1), td(1)
    return new ceres::AutoDiffCostFunction<ICPCostFunction_p2p, 3, 7, 7, 7, 1, 1>(
        new ICPCostFunction_p2p(
            _pts_i, _velocity_i, _td_i,  // anchor frame parameters
