...
Eigen::Matrix2d ProjectionTwoFrameTwoCamFactor::sqrt_info; // = I3x3 multiplied by focal lenght/ 1.5
...
{   
    // point i = point at start frame (first apperence of the feature) left frame
    // velocity == feature velocity in the image plane
    velocity_i.x() = _velocity_i.x();
    velocity_i.y() = _velocity_i.y();
    velocity_i.z() = 0;

    // point j = current observation of the feature right frame
    // velocity == feature velocity in the image plane
    ...
    Eigen::Vector3d a = pts_j.normalized(); // get the unit vector from ? to the feature point
    Eigen::Vector3d tmp(0, 0, 1); // get the z axis of teh camera reference frame
    if(a == tmp)
        tmp << 1, 0, 0;
    // evaluate a base of the tangent plane passing by the point
...
{
    TicToc tic_toc;
    Eigen::Vector3d Pi(parameters[0][0], parameters[0][1], parameters[0][2]); // base link position at start frame
    Eigen::Quaterniond Qi(parameters[0][6], parameters[0][3], parameters[0][4], parameters[0][5]); // base link orientation at start frame

    Eigen::Vector3d Pj(parameters[1][0], parameters[1][1], parameters[1][2]); // base link position at current frame
    Eigen::Quaterniond Qj(parameters[1][6], parameters[1][3], parameters[1][4], parameters[1][5]); // base link orientation at current frame

    Eigen::Vector3d tic(parameters[2][0], parameters[2][1], parameters[2][2]); // left camera position in base link frame
    Eigen::Quaterniond qic(parameters[2][6], parameters[2][3], parameters[2][4], parameters[2][5]); // left camera orientation in base link frame

    Eigen::Vector3d tic2(parameters[3][0], parameters[3][1], parameters[3][2]); // right camera position in base link frame
    Eigen::Quaterniond qic2(parameters[3][6], parameters[3][3], parameters[3][4], parameters[3][5]); // right camera orientation in base link frame

    double inv_dep_i = parameters[4][0]; // inverce of the depth feature

    double td = parameters[5][0]; // time offset 

    Eigen::Vector3d pts_i_td, pts_j_td;
    pts_i_td = pts_i - (td - td_i) * velocity_i; // project the point in the left image to the current image location assuming constant speed motion of the feature point
    pts_j_td = pts_j - (td - td_j) * velocity_j; // same on the right observation between its time observat

    Eigen::Vector3d pts_camera_i = pts_i_td / inv_dep_i; // convert in 3d point
    Eigen::Vector3d pts_imu_i = qic * pts_camera_i + tic; // project in base link frame
    Eigen::Vector3d pts_w = Qi * pts_imu_i + Pi; // project in word frame
    Eigen::Vector3d pts_imu_j = Qj.inverse() * (pts_w - Pj); // from the point in world frame obrained from point i, reproject the point in the camera frame with the informations obtained from the right camera
    Eigen::Vector3d pts_camera_j = qic2.inverse() * (pts_imu_j - tic2); // project the obtained point on the right camera frame
    Eigen::Map<Eigen::Vector2d> residual(residuals); // init residual vector

#ifdef UNIT_SPHERE_ERROR // if unit sphere case (only mono case i guess)
    residual =  tangent_base * (pts_camera_j.normalized() - pts_j_td.normalized()); // evaluate the residual as projection on the tanjent plane (see paper)
#else
    double dep_j = pts_camera_j.z(); // get the depth that the point should have based on the previous evaluations
    residual = (pts_camera_j / dep_j).head<2>() - pts_j_td.head<2>(); // evaluate image reprojection error
#endif
