...
void pubOdometry(const Estimator &estimator, const std_msgs::msg::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        Eigen::Vector3d new_pose(estimator.Ps[WINDOW_SIZE].x(), estimator.Ps[WINDOW_SIZE].y(), estimator.Ps[WINDOW_SIZE].z());

        // Initialize all to 0
        std::fill(odometry.pose.covariance.begin(), odometry.pose.covariance.end(), 0.0);
        
        // Set diagonal elements (variances)
        odometry.pose.covariance[0] = 0.001;   // variance in x (m^2)
        odometry.pose.covariance[7] = 0.001;   // variance in y (m^2)
        odometry.pose.covariance[14] = 0.001;  // variance in z (m^2)
        odometry.pose.covariance[21] = 0.001;   // variance in rotation about x (rad^2)
        odometry.pose.covariance[28] = 0.001;   // variance in rotation about y (rad^2)
        odometry.pose.covariance[35] = 0.01;   // variance in rotation about z (rad^2)
        
        // Initialize all to 0
        std::fill(odometry.twist.covariance.begin(), odometry.twist.covariance.end(), 0.0);
        
        // Set diagonal elements (variances)
        odometry.twist.covariance[0] = 0.001;   // variance in vx (m^2/s^2)
        odometry.twist.covariance[7] = 0.001;   // variance in vy (m^2/s^2)
        odometry.twist.covariance[14] = 0.001;  // variance in vz (m^2/s^2)
        odometry.twist.covariance[21] = 0.001;   // variance in rotation rate about x (rad^2/s^2)
        odometry.twist.covariance[28] = 0.001;   // variance in rotation rate about y (rad^2/s^2)
        odometry.twist.covariance[35] = 0.01; 
        
        ///// LOG /////
        std::ostringstream oss;
        double time_odom = header.stamp.sec + header.stamp.nanosec * (1e-9);
        oss << "=========================================================================\nV odometry at time " << std::setprecision(15) << time_odom << std::endl;
        oss << "x: " << odometry.pose.pose.position.x << " y: " << odometry.pose.pose.position.y << " z: " << odometry.pose.pose.position.z << std::endl;
        logMessage(oss.str());
        ///// LOG /////
...
void pubTF(const Estimator &estimator, const std_msgs::msg::Header &header)
{
    ...
    transform_cam.header.frame_id = "body_link";
    transform_cam.child_frame_id = ref_frame["left_camera"].header.frame_id;
...
//     q.setZ(Quaterniond(estimator.ric[0]).z());
//     transform.setRotation(q);
//     // br->sendTransform(tf2::StampedTransform(transform, header.stamp, "body", ref_frame["left_camera"].header.frame_id));
//     br->sendTransform(tf2::StampedTransform(transform, header.stamp, "body", ref_frame["left_camera"].header.frame_id));
...
void pubTreeMatchImage(const cv::Mat &imgTreeMatch, const sensor_msgs::msg::CameraInfo &camera_info, const double t)
{   
    // create a header (common for camera info and image)
    std_msgs::msg::Header header = camera_info.header;
    ...
    sensor_msgs::msg::CameraInfo info_msg = camera_info;
    info_msg.header = header;
    info_msg.height = imgTreeMatch.rows;
    info_msg.width = imgTreeMatch.cols;

    // create the image message 
    sensor_msgs::msg::Image::SharedPtr imgTrackMsg = cv_bridge::CvImage(header, "bgr8", imgTreeMatch).toImageMsg();
    
    // publish the messages
    pub_tree_match->publish(*imgTrackMsg);
    pub_tree_match_info->publish(info_msg);
}